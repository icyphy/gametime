...
def compute_histogram(
    paths: list[Path], bins=10, path_value_range=None, measured=False
):
    """
    ...
    Parameters:
        paths: list[Path] :
            List of feasible paths generated by GameTime, each represented by a `~gametime.src.path.Path` object.
        bins:
    ...
    """
    path_values = [
        path.measured_value if measured else path.predicted_value for path in paths
    ]
...
def write_histogram_to_file(
    location, paths, bins=10, path_value_range=None, measured=False
):
    """
    ...
    try:
        ...
        err_msg = "Error writing the histogram to the file located " "at %s: %s" % (
            location,
            e,
        )
