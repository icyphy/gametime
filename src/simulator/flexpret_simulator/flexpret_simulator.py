...
class FlexpretSimulator(Simulator):
    ...
    def object_file_to_mem (self, object_file_name):
        ...
        print("obj: ", object_file_name)
        print("scr: ", app_sources)
        print("path: ",os.path.join("..", self.projectConfig.gametime_file_path, self.projectConfig.gametime_flexpret_path))

        os.chdir(cwd)

        mem_file_path = self.projectConfig.get_temp_filename_with_extension(".mem", object_file_name)
        print("path mem: ",
              mem_file_path)
    ...
    def _runSimulatorAndParseOutput(self, mem_file_name):
    ...
    def measure(self, file_name):
        cycleCount = -1
        try:
            self.object_file_to_mem(file_name)
            cycleCount = self._runSimulatorAndParseOutput(file_name)
            # self._removeTemps()
