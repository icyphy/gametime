...
class FlexpretSimulator(Simulator):
    ...
    def object_file_to_mem (self, object_file_name):
        ...
        for entry in context_folder:
            if (not entry == self.projectConfig.nameOrigFile) and (entry.endswith('.c') or entry.endswith('.h') or entry.endswith('.o')):
        ...
        os.system(f'make FLEXPRET_ROOT_DIR=../../../../../flexpret/ NAME=add APP_SOURCES={"../../../test/test_flexpret_simulator/programs/add/add.c"}')
        # os.system(f'make FLEXPRET_ROOT_DIR={flexpret_path_from_flexpret_simulator} NAME=riscv-gt APP_SOURCES={app_sources}')
