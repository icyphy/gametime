...
class PathAnalyzer(object):
    ...
    def __init__(self, analyzer: Analyzer, path: Path, path_name: str):
        """
        used to run the entire simulation on the given path.

        :param analyzer: the analyzer that corresponds to the program being analyzed; PATH should be a path object of it
        ...
        self.project_config: ProjectConfiguration = analyzer.project_config
        self.path: Path = path
    ...
    def change_bt_based_on_path(self) -> str:
        ...
        """
        # read from file
        with open(self.analyzer.preprocessed_path, "r") as preprocessed_file:
        ...
        for node in self.path.nodes:
            node_label: str = self.analyzer.dag.get_node_label(self.analyzer.dag.nodes_indices[node])
