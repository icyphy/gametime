...
def run_gametime(
    ...
    try:
        ...
        project_config: ProjectConfiguration = YAMLConfigurationParser.parse(
            config_path
        )
        ...
        if not project_config.backend:
            ...
            logger.error(
                "Please specify a backend in the config file or use --backend option"
            )
        ...
        if clean_temp and os.path.exists(project_config.location_temp_dir):
            logger.info(
                f"Cleaning temporary directory: {project_config.location_temp_dir}"
            )
        ...
        logger.info(
            f"Backend: {project_config.backend if project_config.backend else 'default'}"
        )
        ...
        if results:
            ...
            max_path = [
                name
                for name, _, measured_value in results
            ...
            for i, (name, predicted_value, measured_value) in enumerate(results):
                ...
                logger.info(
                    f"  {i}: {name} = predicted: {predicted_value}, measured: {measured_value}{marker}"
                )
...
Examples:
  ...
  gametime /path/to/test/folder --visualize-weights
    ...
    parser.add_argument("--version", action="version", version="GameTime 0.1.0")
    ...
    if not config_path:
        ...
        logger.error(
            "Please ensure the directory contains a config.yaml or config.yml file"
        )
        return 1
