import subprocess
import unittest
import os
from os.path import exists

import clang_helper
import nx_helper
from project_configuration import ProjectConfiguration
from project_configuration_parser import YAMLConfigurationParser


class TestGenerateCFG(unittest.TestCase):
    def setUp(self):
        self.project_config: ProjectConfiguration = \
            YAMLConfigurationParser.parse("test_analyzer/programs/test1/config.yaml")

    # self.project_config.location_orig_file, self.project_config.location_temp_dir, f"{self.project_config.name_orig_no_extension}gt", flexpret_lib_path


    def test_generate_cfg_from_c_file(self):
        source_path = os.path.relpath("test_analyzer/programs/tests1")
        source_file = os.path.join(source_path, "main.c")
        compile_command = ["clang", "-emit-llvm", "-o", "test_analyzer/programs/object.bc", "-c", source_file]
        exit_code = subprocess.run(compile_command)
        print(exit_code)
        print(source_file, os.path.isfile(source_file))

    def test_generate_bc_from_c_file_using_clang_helper(self):
        output_path: str = clang_helper.compile_to_llvm(self.project_config.location_orig_file, self.project_config.location_temp_dir,"compile-gt","")
        self.assertTrue(os.path.isfile(output_path))
        self.assertEqual("compile-gt.bc", os.path.split(output_path)[1])

    def test_generate_dot_from_c_file_using_clang_helper(self):
        output_path: str = clang_helper.compile_to_llvm(self.project_config.location_orig_file, self.project_config.location_temp_dir,"compile-gt","")
        output_path = clang_helper.generate_dot_file(output_path, self.project_config.location_temp_dir)
        self.assertTrue(os.path.isfile(output_path))
        self.assertEqual(os.path.split(output_path)[1], ".main.dot")

    def test_read_dag_from_dot_file_compiled_from_c(self):
        output_path: str = clang_helper.compile_to_llvm(self.project_config.location_orig_file, self.project_config.location_temp_dir,"compile-gt","")
        output_path = clang_helper.generate_dot_file(output_path, self.project_config.location_temp_dir)
        dag: nx_helper.Dag = nx_helper.construct_dag(output_path)
        self.assertIsNotNone(dag)
        # self.assertTrue(nx_helper.has_cycles(dag))
        self.assertIsNotNone(nx_helper.get_random_path(dag, dag.source, dag.sink))

    def test_read_loop_unrolled_dag_from_dot_file_compiled_from_c(self):
        output_path: str = clang_helper.compile_to_llvm(self.project_config.location_orig_file, self.project_config.location_temp_dir,"compile-gt","")
        output_path = clang_helper.inline_functions(output_path, self.project_config.location_temp_dir,"inline-gt")
        output_path = clang_helper.unroll_loops(output_path, self.project_config.location_temp_dir,"unroll-gt")
        output_path = clang_helper.generate_dot_file(output_path, output_path, self.project_config.location_temp_dir)
        dag: nx_helper.Dag = nx_helper.construct_dag(output_path)
        self.assertIsNotNone(dag)
        self.assertFalse(nx_helper.has_cycles(dag))
        self.assertIsNotNone(nx_helper.get_random_path(dag, dag.source, dag.sink))
        self.assertEqual(nx_helper.num_paths(dag, dag.source, dag.sink), 1)

    def test_read_write_dag(self):
        output_path: str = clang_helper.compile_to_llvm(self.project_config.location_orig_file,
                                                        self.project_config.location_temp_dir, "compile-gt", "")
        output_path = clang_helper.generate_dot_file(output_path, self.project_config.location_temp_dir)
        dag: nx_helper.Dag = nx_helper.construct_dag(output_path)
        output_dag_path: str = self.project_config.get_temp_filename_with_extension(".dot", "test_output")
        nx_helper.write_dag_to_dot_file(dag, output_dag_path)
        self.assertTrue(exists(output_dag_path))

    def test_compile(self):
        output_path: str = clang_helper.compile_to_llvm(self.project_config.location_orig_file,
                                                        self.project_config.location_temp_dir, "compile-gt", "")
        output_path = clang_helper.inline_functions(output_path, self.project_config.location_temp_dir, "inline-gt")
        output_path = clang_helper.unroll_loops(output_path, self.project_config.location_temp_dir, "unroll-gt")
        clang_helper.compile_to_object_flexpret(output_path, "", "", self.project_config.location_temp_dir, "object-gt")

    def test_clean_temp_dir(self):
        clang_helper.remove_temp_cil_files(self.project_config)

    def test_clean_temp_all(self):
        clang_helper.remove_temp_cil_files(self.project_config, all_temp_files=True)


if __name__ == '__main__':
    unittest.main()
